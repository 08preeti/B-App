...
function BillFormFields({ f, set, cats, showAddCat, setShowAddCat, newCategory, setNewCategory, onAddCategory }) {
  return (
        ...
        {showAddCat && (
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <input
              ...
              style={{ flex: 1 }}
            />
            <button className="primary-btn" onClick={onAddCategory}>Add</button>
            <button className="cancel-btn" onClick={() => setShowAddCat(false)}>✕</button>
          </div>
        )}
      </div>
      <div className="field full">
  ...
  return (
    <div className="page">
      ...
      <div className="table-card">
        {filtered.length === 0 && (
          <div className="empty-state">No bills found. Click <strong>+ Add Bill</strong> to get started.</div>
        )}
        {filtered.map(bill => {
          const id = bill._id || bill.id;
          const color = CAT_COLORS[bill.category] || "#64748b";
          return (
            <div key={id} className="bill-row" onClick={() => setDetail(bill)}>
        ...
        <div className="modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="field full" style={{ marginBottom: 16 }}>
                ...
                <div
                  ...
                  onDrop={e => {
                    ...
                    if (f && f.size <= 5 * 1024 * 1024) { setUploadedFile(f); runOCR(f, setForm); }
                  }}
              ...
              <BillFormFields
                f={form} set={setForm} cats={categories}
                showAddCat={showAddCat} setShowAddCat={setShowAddCat}
      ...
      {editBill && (
        <div className="modal-overlay" onClick={() => setEditBill(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <BillFormFields
                f={editForm} set={setEditForm} cats={categories}
                showAddCat={showAddCat} setShowAddCat={setShowAddCat}
                newCategory={newCategory} setNewCategory={setNewCategory}
                onAddCategory={handleAddCategory}
              />
